-- Exclude AI handoff messages ("Nome Sobrenome Telefone: wa.me/...") from Ramon phone extraction
...
begin
  ...
  with matches as (
    ...
    where h.message is not null
      ...
      -- Bloqueia formato da IA: "Nome Sobrenome Telefone: ..." (T maiúsculo + dois-pontos)
      and h.message !~ '\S+\s+\S+\s+Telefone:\s*'
  ),
  normalized as (
    select
      created_at,
