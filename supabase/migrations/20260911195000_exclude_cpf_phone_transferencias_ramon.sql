-- Strengthen CPF scrub: labeled CPF with 10–11 digits (agents often omit a digit) + formatted xxx.xxx.xxx-xx.
  ...
  with matches as (
    ...
    cross join lateral regexp_matches(
      regexp_replace(
        regexp_replace(
          coalesce(h.message, ''),
          -- CPF label + 10–11 digits (raw) OR formatted; also bare xxx.xxx.xxx-xx
          '(?:CPF[\s:./-]*(?:[0-9]{3}[.\s-]?[0-9]{3}[.\s-]?[0-9]{3}[-\s]?[0-9]{2}|[0-9]{10,11})|[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2})',
          ' ',
          'gi'
        ),
